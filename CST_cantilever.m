...
dofs_free = [1:(floor(ny/2)+1)*2-2,(floor(ny/2)+1)*2+1:N*2-(floor(ny/2)+1)*2+1,N*2-(floor(ny/2)+1)*2+3:2*N]; % unknown nodal x-y dofs
dofs_restrained = [(floor(ny/2)+1)*2-1,(floor(ny/2)+1)*2,N*2-(floor(ny/2)+1)*2+2]; % known nodal x-y dofs due to BC (at nodes 1,9)
...
F(2*N-1) = 1e5; % The load P acts downwards on node 26 i.e. it affects global dof 2*26
F(2*N-ny*2-1) = -1e5;
...
% disp('Total applied vertical loads = 0');
% if abs(fR(2)+fR(3)) < 1e-6; disp('Ok.'); end
% disp(' '); disp('Horizontal equilibrium check:');
% disp(['Total horizontal reactions = ',num2str(fR(1))]);
% disp(['Total applied horizontal loads = ',num2str(F(2*N-ny*2-1)+F(2*N-1))]);
% if abs(fR(1)+F(2*N-ny*2-1)+F(2*N-1)) < 1e-6; disp('Ok.'); end
