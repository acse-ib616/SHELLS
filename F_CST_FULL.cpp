...
public:
    void operator()(matlab::mex::ArgumentList outputs, matlab::mex::ArgumentList inputs)
    {
        ...
        double weight = inputs[1][0]; // Unit weight of material N/m3
        double Ly = inputs[2][0];     // Height of domain mm
        ...
        matlab::data::TypedArray<double> q_Array = std::move(inputs[3]);
        std::vector<double> q(q_Array.begin(), q_Array.end()); // Node coordinates
    ...
    void UDL(double t, double Ly, double weight, std::vector<double> q, std::vector<double> node_coords, std::vector<uint64_t> ELEMENTS, std::vector<double> &forces)
    {
