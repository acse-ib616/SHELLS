...
public:
    void operator()(matlab::mex::ArgumentList outputs, matlab::mex::ArgumentList inputs)
    {
        ...
        double q = inputs[1][0];      // UDL in N/m
        double weight = inputs[2][0]; // Unit weight of material N/m3
        double Ly = inputs[3][0];     // Height of domain mm
        ...
        std::vector<double> F(n_dofs, 0);     // Nodal forces vector

        // Call function to assemble K
        UDL(t, Ly, weight, q, node_coords, ELEMENTS, F);

        matlab::data::ArrayFactory fact;

        // Assign MEX function output
        outputs[0] = fact.createArray<double>({F.size(), 1}, F.data(), F.data() + F.size());
    }
    ...
    void UDL(double t, double Ly, double weight, double q, std::vector<double> node_coords, std::vector<uint64_t> ELEMENTS, std::vector<double> &forces)
    {
        ...
        for (uint64_t i = 0; i < els; i++)
        {
            ...
            if (y1 == Ly && y3 == Ly)
            {
                forces[dof12] = forces[dof12] + q * abs(x31) * 1e-3 / 2;
                forces[dof32] = forces[dof32] + q * abs(x31) * 1e-3 / 2;
            }
        }
    };
};